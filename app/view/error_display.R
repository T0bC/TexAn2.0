...
#' @param icon_name Character, Bootstrap icon name
...
render_app_error <- function(error_obj, show_icon = TRUE,
  ...
  if (!is.null(error_obj$context) && length(error_obj$context) > 0) {
    context_items <- lapply(names(error_obj$context), function(key) {
      ...
      # Format value — MUST be length-one character for Shiny tags
      formatted_value <- if (is.logical(value)) {
        if (length(value) > 1) {
          paste(ifelse(value, "Yes", "No"), collapse = ", ")
        } else {
          ifelse(value, "Yes", "No")
        }
...
#'
#' Displays a structured error in a modal dialog. Useful for critical
#' errors that need user acknowledgment.
#'
#' @param error_obj Structured error object from create_app_error()
#' @param title Character, modal title
#' @param session Shiny session object
#' @export
show_error_modal <- function(error_obj, title = "Error",
