...
# =============================================================================
# Pure logic functions for Summary
...
validate_inputs <- function(columns, data) {
  if (is.null(columns) || length(columns) == 0) {
    rhino$log$warn("Summary: no columns selected")
    return(list(
      valid = FALSE,
...
#' Run a summary computation wrapped in safe_execute
...
run_analysis <- function(data, columns) {
  error_handling$safe_execute(
    expr = {
      ...
      rhino$log$info(
        "Summary: analysis complete ({length(columns)} columns)"
      )
      subset
    },
    operation_name = "summary_analysis"
  )
}
