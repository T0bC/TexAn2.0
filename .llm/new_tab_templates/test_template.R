...
describe("validate_inputs", {
  it("returns valid = TRUE for valid columns", {
    ...
    result <- {tab_name}$validate_inputs(c("a", "b"), data)
    expect_true(result$valid)
  })

  it("returns valid = FALSE when no columns selected", {
    data <- data.frame(a = 1:3)
    result <- {tab_name}$validate_inputs(NULL, data)
    expect_true(!result$valid)
  })

  it("returns valid = FALSE for missing columns", {
    data <- data.frame(a = 1:3)
    result <- {tab_name}$validate_inputs(c("a", "z"), data)
    expect_true(!result$valid)
  })
})
...
describe("run_analysis", {
  it("returns success for valid input", {
    ...
    result <- {tab_name}$run_analysis(data, c("a", "b"))
    expect_true(result$success)
  })

  it("returns error for invalid columns", {
    data <- data.frame(a = 1:3)
    result <- {tab_name}$run_analysis(data, c("nonexistent"))
    expect_true(!result$success)
  })
})
